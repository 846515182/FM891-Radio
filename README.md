...
- 🔎 **发现电台**：接入 Radio Browser 全球电台目录（8 万+，radiocast.co 同款数据源），
  支持按名称搜索、风格标签（流行/摇滚/爵士/电子/古典/民谣/华语）、中国台筛选，
  点击结果即可播放并自动加入频道列表
...
- "发现电台"的数据来自开源目录 [Radio Browser](https://www.radio-browser.info/)，
  网页会自动过滤非 HTTPS 直播源（否则 https 页面无法播放），并在多个镜像间自动切换。
