...
/* ---------------- 发现电台（Radio Browser 目录，radiocast.co 同款数据源） ---------------- */
const API_MIRRORS = [
...
async function searchStations(query) {
  ...
  let lastError = null;
  for (const server of shuffled(API_MIRRORS)) {
    try {
      const ctrl = new AbortController();
      const timer = setTimeout(() => ctrl.abort(), 8000);
      const res = await fetch(server + '/json/stations/search?' + params.toString(), {
        signal: ctrl.signal,
      });
      clearTimeout(timer);
      if (!res.ok) continue;
  ...
  throw lastError || new Error('电台目录暂无可用镜像');
}

function stationMeta(s) {
  const parts = [];
  if (s.countrycode) parts.push(String(s.countrycode).toUpperCase());
  const tags = String(s.tags || '').split(',').map((t) => t.trim()).filter(Boolean).slice(0, 3);
  if (tags.length) parts.push(tags.join(' / '));
  if (s.bitrate) parts.push(s.bitrate + ' kbps');
  return parts.join(' · ') || '网络电台';
}

function renderResults(raw) {
  searchResultsEl.innerHTML = '';

  // 网页部署在 https 时，http 直播源会被浏览器拦截，所以只保留 https
  const list = raw.filter((s) => String(s.url_resolved || s.url || '').startsWith('https://'));
  const hidden = raw.length - list.length;

  if (!list.length) {
    searchHint.textContent = '没有可用的 HTTPS 电台，换个关键词试试';
    return;
  }

  searchHint.textContent =
    '找到 ' + list.length + ' 个电台' +
    (hidden > 0 ? '（已过滤 ' + hidden + ' 个非 HTTPS 电台）' : '') +
    ' · 点击即可播放';
  ...
  list.forEach((s) => {
    ...
    btn.innerHTML =
      '<span class="dot"></span>' +
      '<span class="info"><b></b><small></small></span>' +
      '<span class="tag">▶ 播放</span>';
    btn.querySelector('b').textContent = s.name || '未命名电台';
    btn.querySelector('small').textContent = stationMeta(s);
    if (url === current().url) {
      btn.classList.add('active');
      btn.querySelector('.tag').textContent = '播出中';
    }
    btn.addEventListener('click', () => addAndPlay(s));
    searchResultsEl.appendChild(btn);
  });
}

function addAndPlay(s) {
  const url = s.url_resolved || s.url;
  if (!url) return;

  let i = playlist.findIndex((p) => p.url === url);
  if (i < 0) {
    playlist.push({
      id: s.stationuuid || 'search-' + Date.now(),
      name: String(s.name || '未命名电台').slice(0, 42),
      desc: stationMeta(s),
      url,
      custom: true,
    });
...
async function runSearch(query) {
  ...
  try {
    const raw = await searchStations(query);
    renderResults(raw);
  } catch (_) {
    searchHint.textContent = '搜索失败：网络问题或电台目录暂不可用，请稍后再试';
  } finally {
...
document.querySelectorAll('#tagChips .chip').forEach((chip) => {
  chip.addEventListener('click', () => {
    document.querySelectorAll('#tagChips .chip').forEach((b) => b.classList.remove('on'));
    chip.classList.add('on');
    runSearch({ tag: chip.dataset.tag, countrycode: chip.dataset.countrycode });
  });
});
...
function currentVersion() {
  ...
  return '1.1'; // 网页版：与 manifest versionName 同步维护
}
